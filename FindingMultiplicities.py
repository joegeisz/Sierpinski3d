...
def iterate_geometry(params):
    ...
    for i, flag in enumerate(done_flags):
        while flag == 0:
            if new_params[indicies[i]] < 48:
                yield new_params
                new_params[indicies[i]] += 1
            else:
                flag = 1
...
for params in geometries:
    ...
    for i in range(48):
        ...
        if numsame == ptcld.shape[0]:
            ...
            number_symmetries += 1
            print("\t contains symmetry: " + str(i))
            print(O(i).rotation_matrix())
            O(i).visualize()
            for i in iterate_geometry(params):
                print(i)
            #break
